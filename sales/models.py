...
class Customer(models.Model):
    profile_code = models.CharField(max_length=50, unique=True, blank=True, null=True)
...
class Product(models.Model):
    ...
    description = models.TextField(blank=True, null=True)
    is_active = models.BooleanField(default=True)
    def __str__(self): return f"{self.name} (Tồn kho: {self.quantity_in_stock})"

class GiftCard(models.Model):
    name = models.CharField(max_length=100)
    value = models.DecimalField(max_digits=12, decimal_places=2)
    is_active = models.BooleanField(default=True)
    def __str__(self): return f"{self.name} - {self.value:,.0f}đ"
...
class Payment(models.Model):
    ...
    def __str__(self): return f"Thanh toán {self.amount_paid} cho hóa đơn #{self.invoice.id}"
